...
public enum TipoDiagnostico {
    ...
    public static TipoDiagnostico fromNombre(String nombre) {
        for (TipoDiagnostico diagnostico : TipoDiagnostico.values()) { // Itera sobre todos los valores del enum
            if (diagnostico.getNombre().equalsIgnoreCase(nombre)) {
                return diagnostico; // Retorna el enum si encuentra coincidencia
            }
        }
        throw new IllegalArgumentException("Diagnóstico no reconocido: " + nombre); // Si no encuentra coincidencia, lanza una excepción
    }
